...
public class OthelloAI
{
    ...
    public (int x, int y) AI(List<(int x, int y)> _validMoves, MainGameData _gamedata, bool _isDebug)
    {
        ...
        int me = _gamedata._turnNum;

        (int, int) bestMove = (0, 0);//仮です
        _recursionsCount = 0;
        StartAILog();

        _maxDepth = _gamedata._AIStrength
        switch
        { AIStrength.nuub => 2, AIStrength.normal => 4, AIStrength.expert => 6, AIStrength.professional => 8, _ => 4 };
        ...
        foreach (var move in _validMoves)
        {
            ...
            int score = -Negamax(newGameData, _depth - 1, int.MinValue, int.MaxValue, false, me);
            if (score > maxScore)
            {
    ...
    private int Negamax(MainGameData _gamedata, int _depth, int alpha, int beta, bool trigger, int me)
    {
        ...
        {
            _gamedata._gameTurn = _depth % 2 == 0 ? GameTurn.prayer : GameTurn.AI;
            _gamedata._turnNum = _depth % 2 == 0 ? 1 : 2;
        }


        if (_depth <= 0)//深さ0は評価値を返す
        {
            int evaluation = EvaFunction(_gamedata, me);
        ...
        if (validMoves.Count == 0)
        {
            ...
            if (trigger)
            {
                ...
                return EvaFunction(_gamedata, me);
            }

            //スキップする処理
            _recursionsCount++;
            DebugLog($"{indent}▶ スキップ発生");
            int score = -Negamax(_gamedata, _depth - 1, -beta, -alpha, true, me);
        ...
        foreach (var move in validMoves)
        {
            ...
            int score = -Negamax(newGameData, _depth - 1, -beta, -alpha, false, me);
